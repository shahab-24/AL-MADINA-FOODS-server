...
async function run() {
  try {
    ...
    app.get("/donor-stats", (req, res) => {
      const donatorEmail = req.query?.email; // Get the email from the query string
      console.log("Request for data with email:", donatorEmail);
      // ... Logic to fetch donor stats from a database or other source using donatorEmail
      if (!donatorEmail) {
        res.status(400).send("Please send email in query params");
      } else {
        //logic for finding the stats
        //sample
        const donorStats = {
          email: donatorEmail,
          donations: 10,
        ...
        res.json(donorStats); // Send the data back as JSON
      }
    });
    app.get("/donor-stats/all", async (req, res) => {
      try {
        const allDonors = await foodCollection
          .aggregate([
            {
              $group: {
    ...
    app.get('/myRequest', verifyToken, async(req, res) => {
        ...
        const query = {"requestedBy.userEmail": userEmail}
        const result = await foodCollection.find(query).toArray()
        res.send(result)
    }
    )
    ...
    app.post("/request-food", verifyToken,  async (req, res) => {
      ...
      const query = {
        ...
        "foodUser.foodStatus": "available",
      };

      const userRequest = await foodCollection.find(query).toArray()
      const alreadyRequestedFood = userRequest.some(request => request.foodId === foodId)

      if(alreadyRequestedFood){
        return res.status().send({message: `You have already requested  the item ${foodName}`})
      }
      // const query = {_id: new ObjectId(foodId)}
